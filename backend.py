...
# =========================
# Itinerary Agent - initial generation + token-efficient HITL revision
# =========================
def itinerary_agent(state: TravelState):
    human_feedback = state.get("human_feedback", "").strip()
    previous_itinerary = state.get("itinerary", "").strip()

    if human_feedback and previous_itinerary:
        # Revision path:
        # Reuse the existing draft instead of sending all specialist
        # results again. This significantly reduces prompt size.
        prompt = f"""
Revise the existing travel itinerary according to the human feedback.
...
- Apply the human feedback specifically and completely.
- Preserve useful parts of the previous itinerary unless the feedback
  requires changing them.
- Do not ignore any reasonable requested change.
- Keep the itinerary practical and budget-aware.
- Do not invent unsupported flight, hotel, weather, or pricing details.
- Return a complete revised itinerary, not a list of changes.
- The revised itinerary must be ready for another human review.
"""
    else:
        # Initial generation path:
        # Specialist-agent results are needed to create the first draft.
        prompt = f"""
...
Use the available specialist results as supporting information.
Do not invent unsupported flight, hotel, weather, or pricing details.
...
# =========================
# Final Response Agent - token-efficient finalization
# =========================
...
Use the approved draft itinerary as the primary source of truth.
Do not invent new flight, hotel, weather, or pricing details.
Preserve the important decisions and recommendations from the draft.
...
- Preserve the approved draft's facts and estimates.
- Mention that live flight APIs may not provide ticket prices when pricing is unavailable.
- Include weather-based travel advice when it is present in the draft.
